 ...
 */ 
#include "led.h"
volatile uint8 interrupt_happend = FALSE ; 
...
void yellow_cgo_pstop ()
{
			...
			for (uint8 i = 0  ; i < 20 ; i++) // blinking effect 1 sec //
			{
				led_toggle(PORT_A,1);				
			   led_toggle (PORT_B,1);			  
				d_elay_ms(250) ;
...
void c_go_ped_stop ()
{
   ...
   if (interrupt_happend)
   {
	...
	for (uint8 i = 0  ; i < 10 ; i++) // blinking effect 5 sec for yellow LEDS//
	{
		if (interrupt_happend)
		{
...
void yellow_cstop_pgo ()
{
	...
	for (uint8 i = 0  ; i < 20 ; i++) // blinking effect 5 sec for yellow LEDS //
	{
		led_toggle(PORT_A,1);
		led_toggle (PORT_B,1);
